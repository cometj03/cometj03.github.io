...
앱을 만들기 전에 어떤 기능이 필요할지 생각해 보았다. 우선 배달의 민족 앱처럼 메뉴 리스트를 보여주고 선택해서 장바구니에 담는 기능과 나가야 할 식기 세팅의 종류와 개수를 알려주는, 가장 기본적인 것부터 구현해야할 것이다. 사실 이렇게만 보면 앱이 재미가 없어서 여기에 추가적인 기능들도 시도해보고 싶다. 예를 들어 이 앱을 직원 모두가 사용한다고 가정했을 때 공지 기능, 로그인 기능, 솔드아웃 기능이 현재로서는 있으면 더 유용한 앱이 될 것 같다.
