...
내가 사용하고 있는 키보드 단축키는 다음과 같다. 이 단축키들은 `AutoHotKey`를 설치한 후, 아래에 첨부된 `.ahk` 확장자 파일[^ahk]을 실행하면 적용할 수 있다. (일일이 마우스로 실행하는 것이 귀찮다면 <kbd>Win</kbd>+<kbd>R</kbd>에서 `shell:startup`를 입력하면 나오는 창에 이 파일을 넣으라. 컴퓨터가 부팅될 때 자동으로 실행될 것이다.)
